...
- A separate state-only split signer-transition reconciliation is permitted only for an explicit, confirmed and idempotent `split` workflow already in `wait_new_signature`. It requires the three distinct recorded key IDs, `parentMode=initial`, a recorded registrar transaction, an exact closed live inventory in which PowerDNS reports the old CSK and replacement KSK as active/published KSKs and the replacement ZSK as active/published ZSK, matching DNSKEY protocol/flags/algorithm, exact new-only InternetX material, exact authoritative and AD-validated new-only parent DS, exact DNSKEY visibility, replacement KSK and ZSK signatures, and unchanged parent delegation. It records no inferred key ID, performs no PowerDNS or registrar write, and starts a new full maximum-zone-TTL plus propagation-margin wait. The same evidence is re-proven before old-key deactivation; TTL growth restarts the wait.
